...
<DIV class='title_line'>
  ...
  <A href="./list_by_bmkgrpno.do?bmkgrpno=${bmkVO.bmkgrpno }" class='title_link'>${bmkgrpVO.name }</A> > 
  ${bmkVO.name } 삭제
</DIV> 
...
<DIV id='panel_delete' style='padding: 10px 0px 10px 0px; background-color: #F9F9F9; width: 100%; text-align: center;'>
    ...
    <FORM name='frm_delete' id='frm_delete' method='POST' action='./delete.do'> <!-- action='./delete.do'  -->
      <input type='hidden' name='bmkno' id='bmkno' value="${bmkVO.bmkno }"> <!-- ★★★ 수정과 삭제시 필요하다.--> 
      <input type='hidden' name='bmkgrpno' id='bmkgrpno' value="${bmkVO.bmkgrpno }"> <!-- ★★★ 수정과 삭제시 필요하다.--> 
      
      <label>그룹 번호</label>: ${bmkVO.bmkgrpno }  
      <label>차량명</label>: ${bmkVO.name}
      <label>내용</label>: ${bmkVO.content}  
